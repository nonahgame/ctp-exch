...
def ai_decision(df, stop_loss_percent=STOP_LOSS_PERCENT, take_profit_percent=TAKE_PROFIT_PERCENT, position=None, buy_price=None):
    ...
    if position == "long" and buy_price is not None:
        ...
        if close_price <= stop_loss:
            ...
            action = "sell"
        elif (lst_diff < 0.00 and macd_hollow >= 0.00 and stoch_rsi >= 0.99 and stoch_k >= 99.98 and stoch_d >= 94.97 and obv >= 819.00 and diff1e > 0.00):
            ...
            action = "sell"
        elif (supertrend_trend == 'up' and stoch_rsi == 1.00 and stoch_k == 100.00 and stoch_d > 90.00 and obv >= 819.00 and diff1e > 0.00 and diff2m > 0.00 and diff3k > 0.00):
            logger.info(
    ...
    if action == "hold" and position is None:
        if (lst_diff >  0.01 and macd_hollow <= 0.01 and stoch_rsi <= 0.01 and stoch_k <= 0.01 and stoch_d <= 25.00 and obv <= -1019.00 and rsi < 24.00):
            logger.info(
            ...
            action = "buy"
        elif (supertrend_trend == 'down' and stoch_rsi <= 0.00 and stoch_k <= 0.00 and stoch_d <= 0.00 and obv <= -1019.00 and diff1e < - 0.00 and diff2m < - 0.00 and diff3k < - 0.00):
            logger.info(
